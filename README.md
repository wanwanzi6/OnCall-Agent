...
当前阶段已完成阶段 5：在阶段 4 稳定规则工作流基础上，`/api/aiops/analyze` 支持 `rule` 和 `agent` 两种模式。默认仍是稳定、可测试、无外部依赖的 `rule` 模式；显式启用 `agent` 模式后，会通过 Eino-compatible Agent 编排调用既有 Alert/Log/Metric provider 和 KnowledgeService，并可在失败时 fallback 到 rule workflow。
